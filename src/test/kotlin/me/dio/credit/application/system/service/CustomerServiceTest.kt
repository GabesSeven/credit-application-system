...
class CustomerServiceTest {
    ...
    fun `should delete customer by id`() {
        ...
        val fakeCustomer: Customer = buildCustomer(id = fakeId)
        every { customerRepository.findById(fakeId) } returns Optional.of(fakeCustomer)
        every { customerRepository.delete(fakeCustomer) } just runs // para apenas rodar e deletar, sem retornar nada
        //when
        customerService.delete(fakeId)
        //then
        verify(exactly = 1) { customerRepository.findById(fakeId) } // como o delete não retorna nada, só deve ser testado se está executando somente uma vez
        verify(exactly = 1) { customerRepository.delete(fakeCustomer) } // como o delete não retorna nada, só deve ser testado se está executando somente uma vez
    }
